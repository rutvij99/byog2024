...
namespace Bosses
{
    public class BlackKnightBoss : BossBase
    {
        [SerializeField] private List<GameObject> stageEffects;
        ...
        IEnumerator ChangeStage()
        {
            ...
            yield return new WaitForSeconds(0.15f);
            foreach (var effect in stageEffects)
            {
                effect.SetActive(true);
            }
            yield return new WaitForSeconds(stageChangeAnim.length - 0.15f);
