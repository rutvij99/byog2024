...
#pragma warning restore 414
    ...
    public class MConditionsEditor : Editor
    {
        SerializedObject so;
        MConditions M;
        SerializedProperty conditions, Then, Else, SelectedState, showResponse, EvaluateOnEnable,
            debug;
        ...
        private void Draw_Header_State(Rect rect)
        {
            ...
            Rect R_3 = new Rect(rect.x, rect.y - 1, 25, EditorGUIUtility.singleLineHeight);
            ...
            if (_OnEnable == null)
            {
                _OnEnable = new GUIContent(EditorGUIUtility.IconContent("d_toggle_on_focus"));
                _OnEnable.tooltip = "Evaluate all conditions on Enable";
            }

            if (_OnDisable == null)
            {
                _OnDisable = new GUIContent(EditorGUIUtility.IconContent("d_toggle_bg_focus"));
                _OnDisable.tooltip = "Evaluate all conditions on Enable";
            }
            ...
            EvaluateOnEnable.boolValue = GUI.Toggle(R_3, EvaluateOnEnable.boolValue, ON ? _OnEnable : _OnDisable, EditorStyles.label);
            // GUI.color = currentGUIColor;
        }

        private GUIContent _OnEnable, _OnDisable;
